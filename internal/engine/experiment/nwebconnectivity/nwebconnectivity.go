...
type MeasureEndpointConfig struct {
	...
	h3Support  bool
	jar        *cookiejar.Jar
	redirectch chan *NextLocationInfo
	redirected *NextLocationInfo // this field is null for the first request, and contains redirection data for redirected requests
	URL        *url.URL
	wg         *sync.WaitGroup
}
...
func (m *Measurer) runWithRedirect(
	...
	redirectch := make(chan *NextLocationInfo, len(epnts)*2+1)
